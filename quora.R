...
library(text2vec)
library(pryr) #provides combined memory of all objects in the list
library(data.table)
library(caret)
...
# Construct document term matrix for train_set
t1 = Sys.time()
...
test_set <- itoken(test$question, progressbar = FALSE)
...
#Pruning Vocabulary
# reducing training time and by pruning vocabulary
...
# create dtm for train data using pruned vocabulary vectorizer
...
# creating dtm with 2 gram vectorizer
...
# creating dtm on test data by applying 2 gram vectorizer  
...
dtm_test_tfidf  = create_dtm(test_set, vectorizer) %>% transform(tfidf)
